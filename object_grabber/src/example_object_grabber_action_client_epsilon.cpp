...
// bottom right corner from robot's perspective
...
void set_example_object_frames2(geometry_msgs::PoseStamped &object_poseStamped,
    ...
    object_poseStamped.header.frame_id = "system_ref_frame"; //set object pose; ref frame must be connected via tf
    object_poseStamped.pose.position.x = 0.5;
    object_poseStamped.pose.position.y = -0.05;
    object_poseStamped.pose.position.z = 0.7921; //-0.125; //pose w/rt world frame
    object_poseStamped.pose.orientation.x = 0;
    object_poseStamped.pose.orientation.y = 0;
    object_poseStamped.pose.orientation.z = 0.842;
...
void set_example_object_frames3(geometry_msgs::PoseStamped &object_poseStamped,
        geometry_msgs::PoseStamped &object_dropoff_poseStamped) {
    //hard code an object pose; later, this will come from perception
    //specify reference frame in which this pose is expressed:
    //will require that "system_ref_frame" is known to tf
    object_poseStamped.header.frame_id = "system_ref_frame"; //set object pose; ref frame must be connected via tf
    object_poseStamped.pose.position.x = 0.7;
    object_poseStamped.pose.position.y = -0.05;
    object_poseStamped.pose.position.z = 0.7921; //-0.125; //pose w/rt world frame
    object_poseStamped.pose.orientation.x = 0;
    object_poseStamped.pose.orientation.y = 0;
...
void set_example_object_frames4(geometry_msgs::PoseStamped &object_poseStamped,
        geometry_msgs::PoseStamped &object_dropoff_poseStamped) {
    //hard code an object pose; later, this will come from perception
    //specify reference frame in which this pose is expressed:
    //will require that "system_ref_frame" is known to tf
...
int main(int argc, char** argv) {
    ...
    // Long sleep time here - about 2 min 30 sec or so.
    //    ros::Duration(150).sleep(); //Our guess


// bottom right corner from robot's perspective
    ROS_INFO("GRAB 1 ABOUT TO HAPPEN: ");
...
// left and up from original grab
    set_example_object_frames3(object_pickup_poseStamped, object_dropoff_poseStamped);

    ROS_INFO("GRAB 3 ABOUT TO HAPPEN: ");
    grab_object(object_pickup_poseStamped);
    while(!g_got_callback) {
        ROS_INFO("waiting on grab...");
        ros::Duration(0.5).sleep(); //could do something useful
...
// forward of original grab (along right edge of robot's perspective of table)
    set_example_object_frames4(object_pickup_poseStamped, object_dropoff_poseStamped);

    ROS_INFO("GRAB 4 ABOUT TO HAPPEN: ");
    grab_object(object_pickup_poseStamped);
    while(!g_got_callback) {
        ROS_INFO("waiting on grab...");
        ros::Duration(0.5).sleep(); //could do something useful
