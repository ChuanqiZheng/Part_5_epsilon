...
void set_example_object_frames(geometry_msgs::PoseStamped &object_poseStamped,
    ...
    object_poseStamped.pose.position.x = 0.65;
    object_poseStamped.pose.position.y = 0.1;
    object_poseStamped.pose.position.z = 0.66;//0.7921; //-0.125; //pose w/rt world frame
    object_poseStamped.pose.orientation.x = 0;
    object_poseStamped.pose.orientation.y = 0;
    object_poseStamped.pose.orientation.z = 0.7071;
    object_poseStamped.pose.orientation.w = 0.7071;
    ...
    object_dropoff_poseStamped.pose.position.y = -0.5;
    object_dropoff_poseStamped.pose.orientation.z = 0.7071;
    object_dropoff_poseStamped.pose.orientation.w = 0.7071;
}

void set_example_object_frames2(geometry_msgs::PoseStamped &object_poseStamped,
        geometry_msgs::PoseStamped &object_dropoff_poseStamped) {
    //hard code an object pose; later, this will come from perception
    //specify reference frame in which this pose is expressed:
    //will require that "system_ref_frame" is known to tf
    ...
    object_poseStamped.pose.position.y = -0.5;
    object_poseStamped.pose.position.z = 0.66;//0.7921; //-0.125; //pose w/rt world frame
    object_poseStamped.pose.orientation.x = 0;
    object_poseStamped.pose.orientation.y = 0;
    object_poseStamped.pose.orientation.z = 0.7071;
    ...
    object_dropoff_poseStamped = object_poseStamped; //specify desired drop-off pose of object
    object_dropoff_poseStamped.pose.position.y = 0.1;
    object_dropoff_poseStamped.pose.orientation.z = 0.7071;
    object_dropoff_poseStamped.pose.orientation.w = 0.7071;
}
...
void move_to_waiting_pose() {
        ...
        object_grabber_goal.action_code = object_grabber::object_grabberGoal::MOVE_TO_WAITING_POSE;//need to find this hard code!!!
        g_object_grabber_ac_ptr->sendGoal(object_grabber_goal, &objectGrabberDoneCb);//do it!
}
...
void reachingendofhallwayCallback(const std_msgs::Bool& hallway_msg) 
{ 
  g_reaching_end_of_hallway = hallway_msg.data;
}
...
int main(int argc, char** argv) {
    ...
    ROS_INFO("Moving to waiting pose...");
    ...
    //stuck here until jinx arrives end of hallway
     while(!g_reaching_end_of_hallway)
    {
      ros::spinOnce();
      ros::Duration(0.5).sleep(); 
      ROS_INFO("waiting until Jinx reach end of hallway");
    }
    ros::Duration(2.0).sleep(); 
    ...
    ros::Duration(3.0).sleep();
    set_example_object_frames2(object_pickup_poseStamped, object_dropoff_poseStamped);
    grab_object(object_pickup_poseStamped);
    while(!g_got_callback) {
        ROS_INFO("waiting on grab...");
        ros::Duration(0.5).sleep(); //could do something useful
...
lidar_alarm_publisher_.publish(lidar_alarm_msg);*/
    ...
    std_msgs::Bool sweep_finish;
    sweep_finish.data = true;
    for(int i=0;i<10;i++)
    {
      sweepstatus_pub.publish(sweep_finish);
      ros::Duration(0.5).sleep();
    }
