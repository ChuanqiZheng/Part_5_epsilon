// arm7dof_cart_move_as: //NOT DONE...
// wsn,  Nov, 2016
// action server to accept commands and perform planning and motion requests

//for arm7dof, reference link8 for tool flange and link1 for base
...
private:
    ...
    Arm7dof_fwd_solver arm7dof_fwd_solver; //instantiate a forward-kinematics solver 
...
void ArmMotionInterface::executeCB(const actionlib::SimpleActionServer<cartesian_planner::cart_moveAction>::GoalConstPtr& goal) {
    ...
    switch (command_mode_) {
        ...
        case cartesian_planner::cart_moveGoal::PLAN_PATH_CURRENT_TO_WAITING_POSE:
            ...
            ROS_INFO_STREAM("q_start_Xd: " << q_start_Xd_ << endl);
            ROS_INFO("testtesttesttesttest");
...
action_client_("trajActionServer", true)
 { // constructor
    ...
    //A_tool_wrt_flange_ = arm7dof_fwd_solver.get_affine_tool_wrt_flange();
    ...
    while (tferr) {
        tferr=false;
        try {
                tfListener_->lookupTransform("generic_gripper_frame","link8",  ros::Time(0), generic_toolflange_frame_wrt_gripper_frame_stf_);
                ...
                ros::spinOnce();      
                ntries++;
                if (ntries>5) ROS_WARN("did you launch cartesian_planner/arm7dof_static_transforms.launch?");
            }   
    }
...
void ArmMotionInterface::compute_tool_stamped_pose(void) {
    ...
    affine_tool_wrt_base_ =
            arm7dof_fwd_solver.fwd_kin_flange_wrt_base_solve(q_vec_); //rtns flange pose w/rt base frame
    //FIX THIS--add tool-transform
...
void ArmMotionInterface::compute_flange_stamped_pose(void) {
    ...
    affine_flange_wrt_base_ =
            arm7dof_fwd_solver.fwd_kin_flange_wrt_base_solve(q_vec_); //rtns pose w/rt base frame
